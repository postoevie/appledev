...
class FieldsPresenter: FLViewActionsDelegate {
    
    var interaction: FLViewInteraction?
    ...
    func createSections() -> [FLSection] {
        let ratios = getRatios()
        let config = FLFlexItemsConfig(ratios: ratios, rowHeight: 100)
        let section = FLSection(sectionItem: createSectionItem(),
                                type: .flexItems(config),
                                cellItems: createCellItems())
        return [section]
    }
    
    private func createSectionItem() -> FLSectionItem {
    ...
    private func createCellItems() -> [FLCellItem] {
        settingsProvider.getSettingFields().compactMap { field in
            createCellItem(field: field)
        }
    }
    
    private func createCellItem(field: String) -> FLCellItem? {
        ...
        let title = dataService.getTitle(field: field)
        let isReadonly = settingsProvider.isReadonly(field: field)
        let data = FLSingleValueCellData(value: value,
                                         layout: .number,
                                         title: title,
                                         validationErrorText: dataService.getErrorText(field: field),
                                         readonly: isReadonly,
        ...
        return FLCellItem(sectionId: UUID(),
                          uid: uid,
                          data: .titledSingleValue(data),
                          style: style)
    }
    ...
    func updateItems() {
        interaction?.update(cellItems: createCellItems())
    }
}
