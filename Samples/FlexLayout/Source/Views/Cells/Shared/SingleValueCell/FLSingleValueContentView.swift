...
final class FLSingleValueView: NiblessView,
                               UITextFieldDelegate {
    ...
    var appliedConfiguration = FLSingleValueConfiguration(uid: UUID(),
                                                          data: FLSingleValueCellData(value: .empty,
                                                                                      layout: .empty,
                                                                                      accessKey: ""),
                                                          style: FLCellStyle(backgroundColor: .white)) {
        didSet {
    ...
    @objc private func viewAreaTapped(_ recognizer: UITapGestureRecognizer) {
        ...
        case let numeric as FLNumericValueView:
    ...
    private func updateValueView() {
        ...
        case (let view as FLNumericValueView, .text(let string)):
    ...
    private func updateNumericView(_ view: FLNumericValueView, text: String?) {
        view.uid = appliedConfiguration.uid
        view.delegate = appliedConfiguration.delegate
        updateTextField(view.textField, text: text)
        view.setAccessKey(accessKey)
    }
    
    private func createNumericView() -> FLNumericValueView {
        FLNumericValueView(textField: createTextField())
    }
    
    func createAndSetValueView(_ configuration: FLSingleValueConfiguration) {
...
private final class FLNumericValueView: NiblessView {
